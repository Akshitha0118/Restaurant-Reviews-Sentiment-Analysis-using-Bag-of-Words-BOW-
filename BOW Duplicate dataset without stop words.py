# BOW Duplicate dataset /without stop words
...
#df = pd.read_csv("/mnt/data/Restaurant_Reviews.tsv", sep="\t")

# Duplicate rows
df_duplicated = pd.concat([df, df,df], ignore_index=True)
...
for i in range(0, len(df_duplicated)):
    review=re.sub('[^a-zA-Z]', ' ',df_duplicated['Review'][i])
    review=review.lower()
    review=review.split()
    ps=PorterStemmer()
